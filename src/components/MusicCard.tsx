...
export const MusicCard = ({ className, title, artist, coverUrl, onClick }: MusicCardProps) => {
    ...
    return (
        <div onClick={onClick} onMouseEnter={(e) => { e.preventDefault(); setIsHover(true) }} onMouseLeave={(e) => { e.preventDefault(); setIsHover(false) }} className={`relative w-[177px] h-[232px] p-4 ${className}`}>
            ...
            <div className={` w-full h-full flex flex-col gap-2 justify-center shrink-0`}>
                <div className={`w-36 h-36 rounded-xl bg-blue-700 overflow-hidden `}>
...
export const ArtistCard = ({ className, artistName, artistImageUrl }: ArtistCardProps) => {
    const [isHover, setIsHover] = useState(false)

    return (
        <div onMouseEnter={(e) => { e.preventDefault(); setIsHover(true) }} onMouseLeave={(e) => { e.preventDefault(); setIsHover(false) }} className={`relative p-4 w-[177px] h-[232px] ${className}`}>
            ...
            <div className={`${isHover ? "opacity-100" : "opacity-0"} transition-all ease-in-out duration-500 absolute pointer-events-none rounded-lg inset-0 bg-linear-to-t from-gray-500/20 to-transparent`}></div>
            <div className={` w-full h-full flex flex-col gap-2 justify-center shrink-0`}>
                <div className={`w-36 h-36 rounded-full bg-blue-700 overflow-hidden `}>
